...
#         # === Step 1: Build one CART model (single-objective for now) ===
#         model = DecisionTreeRegressor(random_state=random_seed)
#         model.fit(sampled_configs, sampled_performances)

#         # === Step 2: Get remaining (unevaluated) configurations ===
...
#         # === Step 3: Use acquisition function (BAZZA) to pick next config ===
#         selected_idx = acquisition_fn(remaining_configs, model)
#         selected_config = remaining_configs[selected_idx]

#         # === Step 4: Measure true performance ===
...
#         # === Step 5: Update sampled data and results ===
...
# def acquisition_fn(remaining_configs, model, no_directions=10):
#         preds = model.predict(remaining_configs)
#         return np.argmin(preds)  # BAZZA = select config with min predicted performance
