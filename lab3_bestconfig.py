...
def bestconfig_search(file_path, budget, output_file, k=2, top_n=1, random_seed=42):
    """
    使用分治发散采样和递归边界搜索寻找最佳配置

    事实上这个模型很差,因为它的时间复杂度是指数级的,预算只有100
    在大部分情况下,它需要搜索k的8次方个配置(8个配置)，导致预算不足
    就算把k设置为2,它也需要搜索2的8次方个配置(256个配置),超过了100.

    但是假设每个选项对性能的影响是独立的,
    我们在实现bestconfig_search时在dds阶段不搜索k的配置次方数只搜索k*配置数就行了
    这样就能保证时间复杂度是线性的(复杂度从O(k^d)降到O(kd))
    于是我完成了我的线性BestConfigSearch在下一个文件中。
...
# === Stage 1: DDS - Divide and Diverge Sampling 分治发散采样阶段 ===
    intervals = {}  # 创建一个字典来存储每个配置参数的区间划分
    for col in config_columns:  # 遍历每个配置参数列
        unique_vals = sorted(data[col].unique())  # 获取该参数的所有不同取值并排序
        if len(unique_vals) <= k:  # 如果参数的不同取值数量小于等于k（分割数）
            intervals[col] = [[v] for v in unique_vals]  # 则每个值单独作为一个区间
        else:  # 如果参数的不同取值数量大于k
            chunks = np.array_split(unique_vals, k)  # 将取值范围均匀分成k个区间
            intervals[col] = [list(chunk) for chunk in chunks]  # 将每个区间转换为列表并存储

    # 子空间组合（每个参数选择一个interval）
    # 生成所有可能的子空间组合（笛卡尔积）
    subspaces = list(product(*[range(len(intervals[col])) for col in config_columns]))
    np.random.shuffle(subspaces)  # 随机打乱子空间的访问顺序，增加采样多样性
    initial_samples = []  # 存储初始采样的配置

    # 从每个子空间中选择样本
    for i, subspace in enumerate(subspaces):  # 遍历每个子空间
        random_seed_in_loop = random_seed + i  # 为每次迭代生成不同的随机种子
        np.random.seed(random_seed_in_loop)
        if len(initial_samples) >= budget:  # 如果已采样数量达到预算限制则停止
            break
        sample = []  # 存储当前采样的配置
        for idx, col in enumerate(config_columns):  # 遍历每个配置参数
            random_seed_in_loop = random_seed_in_loop + idx  # 为每个参数生成不同的随机种子
            subrange = intervals[col][subspace[idx]]  # 获取该参数在当前子空间中的取值区间
            sample.append(int(np.random.choice(subrange)))  # 从区间中随机选择一个值
        initial_samples.append(sample)  # 将采样的配置添加到初始样本集
    np.random.seed(random_seed)  # 重置随机种子以确保结果可重现
...
if __name__ == "__main__":
    ...
    output_folder = "results\\search_results_bestconfig"  # 输出文件夹
    os.makedirs(output_folder, exist_ok=True)  # 创建输出文件夹（如果不存在）
    budget = 500  # 搜索预算（尝试的配置数量）
